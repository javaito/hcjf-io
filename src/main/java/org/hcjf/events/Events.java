...
public final class Events extends Service<EventListener> {
    ...
    private Events() {
        super(SystemProperties.get(IOSystemProperties.Event.SERVICE_NAME),
                SystemProperties.getInteger(IOSystemProperties.Event.SERVICE_PRIORITY));
    ...
    private void dispatchLocalEvent(Event event) {
        for (EventListener listener : getListeners(event)) {
            try {
                ...
                Log.e(SystemProperties.get(IOSystemProperties.Event.LOG_TAG), "Unable to dispatch event", ex);
            }
        }
    }
