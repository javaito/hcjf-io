...
public class ProcessDiscoveryService extends Service<ProcessDiscoveryConsumer> {
    ...
    private ProcessDiscoveryService() {
        super(SystemProperties.get(IOSystemProperties.ProcessDiscovery.SERVICE_NAME),
                SystemProperties.getInteger(IOSystemProperties.ProcessDiscovery.SERVICE_PRIORITY));
    ...
    private class ProcessMatcher implements Runnable {
        ...
        public void run() {
            while(!shuttingDown) {
                try {
                    ...
                    for (ProcessDiscoveryConsumer consumer : consumers) {
                        try {
                            ...
                            Log.w(SystemProperties.get(IOSystemProperties.ProcessDiscovery.LOG_TAG),
                ...
                try {
                    Thread.sleep(SystemProperties.getLong(IOSystemProperties.ProcessDiscovery.DELAY));
                } catch (InterruptedException e) {
                    break;
                }
            }

            Log.d(SystemProperties.get(IOSystemProperties.ProcessDiscovery.LOG_TAG), "Process matcher end");
        }
