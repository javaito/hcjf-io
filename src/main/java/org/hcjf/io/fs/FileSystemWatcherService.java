...
public final class FileSystemWatcherService extends Service<FileSystemWatcherConsumer> {
    ...
    private FileSystemWatcherService() {
        super(SystemProperties.get(IOSystemProperties.FileSystem.SERVICE_NAME),
                SystemProperties.getInteger(IOSystemProperties.FileSystem.SERVICE_PRIORITY));
        keys = new HashMap<>();
    ...
    public void registerConsumer(FileSystemWatcherConsumer consumer) {
        ...
        synchronized (this) {
            while(watcher == null) {
                try {
                    Log.d(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "Waiting file system watcher init");
                    wait(5000);
        ...
        try {
            synchronized (this) {
                ...
                if(consumer.getTriggerType() == FileSystemWatcherConsumer.TriggerType.WATCHER) {
                    ...
                    Log.i(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "File system watcher registered %s", absolutePath);
            ...
            Log.e(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG),
    ...
    protected void init() {
        ...
        try {
            ...
            Log.d(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "File System Watcher init fail", ex);
        }
    ...
    private class FileSystemPollingTask implements Runnable {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    Thread.sleep(SystemProperties.getLong(IOSystemProperties.FileSystem.POLLING_WAIT_TIME));
                } catch (Exception ex) {
                    Log.e(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG),
                        ...
                        try {
                            ...
                            for(Path checksumPath : lastChecksumMap.keySet()) {
                                try {
                                    ...
                                    Log.w(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG),
            ...
            if(watcher != null) {
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    for(FileSystemWatcherConsumer consumer : consumersForWatch.get(key)) {
                        ...
                        if (!valid) {
                            Log.d(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG),
                ...
                Log.d(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "File system watcher service stopped");
            }
        }
    }
    ...
    private class SystemPropertiesConsumer extends FileSystemWatcherConsumer {
        ...
        private void load(File file) {
            try {
                ...
                Log.w(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "Unable to load properties file", ex);
            }
            Log.d(SystemProperties.get(IOSystemProperties.FileSystem.LOG_TAG), "Properties reloaded");
        }
    }
}
