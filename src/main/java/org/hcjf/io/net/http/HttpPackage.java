...
public abstract class HttpPackage {
    ...
    private void processCookieHeader(HttpHeader httpHeader) {
        try {
            ...
            Log.w(SystemProperties.get(IOSystemProperties.Net.Http.LOG_TAG),
    ...
    public final synchronized void addData(byte[] data) {
        if(!complete) {
            ...
            Log.d(SystemProperties.get(IOSystemProperties.Net.Http.LOG_TAG), "Trying to add data into a complete http package.");
        }
    }
    ...
    private void writeBody(byte[] data, int offset, int length) {
        if(transferDecodingLayer == null) {
            ...
            if(currentBuffer.size() > SystemProperties.getInteger(IOSystemProperties.Net.Http.MAX_PACKAGE_SIZE)) {
                throw new RuntimeException("Http package overflow");
            }
        } else {
            byte[] fragment;
