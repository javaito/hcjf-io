...
public class BroadcastService extends Service<BroadcastConsumer> {
    ...
    static {
        instance = new BroadcastService(SystemProperties.get(IOSystemProperties.Net.Broadcast.SERVICE_NAME));
    ...
    protected void init() {
        ...
        try {
            this.messageDigest = MessageDigest.getInstance(SystemProperties.get(IOSystemProperties.Net.Broadcast.SIGNATURE_ALGORITHM));
        ...
        public void run() {
            ...
            while(!instance.isShuttingDown()) {
                ...
                for(BroadcastConsumer consumer : consumers.values()) {
                    try {
                        ...
                        Log.w(SystemProperties.get(IOSystemProperties.Net.Broadcast.LOG_TAG), ex.getMessage());
                    }
                }

                synchronized (this) {
                    try {
                        this.wait(SystemProperties.getLong(IOSystemProperties.Net.Broadcast.SENDER_DELAY));
            ...
            for(BroadcastConsumer consumer : consumers.values()) {
                try {
                    ...
                    Log.w(SystemProperties.get(IOSystemProperties.Net.Broadcast.LOG_TAG),
                            "", ex);
                }
            }
        }
    ...
    private class BroadcastReceiver implements Runnable {
        ...
        public void run() {
            ...
            while(!instance.isShuttingDown()) {
                try {
                    buffer = new byte[SystemProperties.getInteger(IOSystemProperties.Net.Broadcast.RECEIVER_BUFFER_SIZE)];
