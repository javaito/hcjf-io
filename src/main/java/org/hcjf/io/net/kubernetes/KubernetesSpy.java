...
public final class KubernetesSpy extends Service<KubernetesSpyConsumer> {
    ...
    private KubernetesSpy() {
        super(SystemProperties.get(IOSystemProperties.Net.KubernetesSpy.SERVICE_NAME), 2);
    ...
    private class KubernetesPodSpyTask implements Runnable {
        ...
        public void run() {
            ...
            try {
                namespace = new String(Files.readAllBytes(SystemProperties.getPath(IOSystemProperties.Net.KubernetesSpy.NAMESPACE_FILE_PATH))).trim();
            } catch (Exception ex){}
            ...
            while(!Thread.currentThread().isInterrupted()) {
                ...
                try {
                    ...
                    Log.w(SystemProperties.get(IOSystemProperties.Net.KubernetesSpy.LOG_TAG), "Unable to list pods", ex);
                }
                ...
                try {
                    Thread.sleep(SystemProperties.getLong(IOSystemProperties.Net.KubernetesSpy.TASK_SLEEP_TIME));
                } catch (InterruptedException e) {
                    break;
                }
            }

            Log.d(SystemProperties.get(IOSystemProperties.Net.KubernetesSpy.LOG_TAG), "Kubernetes spy task finish");
        }
    }
}
