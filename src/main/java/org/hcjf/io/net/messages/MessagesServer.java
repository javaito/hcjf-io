...
public abstract class MessagesServer<S extends NetSession> extends NetServer<S, MessageBuffer> {
    ...
    public MessagesServer(Integer port, NetService.TransportLayerProtocol protocol,
        ...
        if(SystemProperties.getBoolean(IOSystemProperties.Net.Messages.SERVER_DECOUPLED_IO_ACTION)) {
            decoupleIoAction(
                    SystemProperties.getInteger(IOSystemProperties.Net.Messages.SERVER_IO_QUEUE_SIZE),
                    SystemProperties.getInteger(IOSystemProperties.Net.Messages.SERVER_IO_WORKERS));
        }
    }
