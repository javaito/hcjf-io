...
public abstract class CloudTimerTask implements Runnable {
    ...
    public CloudTimerTask(String name) {
        ...
        mapName = SystemProperties.get(IOSystemProperties.Cloud.TimerTask.MAP_SUFFIX_NAME) + name;
        lockName = SystemProperties.get(IOSystemProperties.Cloud.TimerTask.LOCK_SUFFIX_NAME) + name;
        conditionName = SystemProperties.get(IOSystemProperties.Cloud.TimerTask.CONDITION_SUFFIX_NAME) + name;

        lock = Cloud.getLock(lockName);
        condition = Cloud.getCondition(conditionName, lock);
        timerTaskMap = Cloud.getMap(SystemProperties.get(IOSystemProperties.Cloud.TimerTask.MAP_NAME));
    }
    ...
    public final void run() {
        ...
        while(!Thread.currentThread().isInterrupted()) {
            ...
            try {
                ...
                delay = getDelay();
                if (delay < SystemProperties.getLong(IOSystemProperties.Cloud.TimerTask.MIN_VALUE_OF_DELAY)) {
                    delay = SystemProperties.getLong(IOSystemProperties.Cloud.TimerTask.MIN_VALUE_OF_DELAY);
                }
                ...
                if (currentExecution.equals(lastExecution)) {
                    ServiceSession previousSession = ServiceThread.getServiceThreadInstance().getSession();
                    //Execute the custom logic
                    try {
                        Log.d(System.getProperty(IOSystemProperties.Cloud.LOG_TAG),"Starting timer task execution %s", name);
                        ServiceThread.getServiceThreadInstance().setSession(ServiceSession.getSystemSession());
                        onRun();
                    } catch (Throwable ex) {
                        Log.d(System.getProperty(IOSystemProperties.Cloud.LOG_TAG),"Starting timer task error execution %s", name);
                        onError(ex);
                    } finally {
                        ServiceThread.getServiceThreadInstance().setSession(previousSession);
                        Log.d(System.getProperty(IOSystemProperties.Cloud.LOG_TAG),"Ending timer task execution %s", name);
                    }
